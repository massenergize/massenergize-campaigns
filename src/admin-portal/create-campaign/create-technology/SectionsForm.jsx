...
export default function SectionsForm({ section, data, updateExistingObject, item_id, apiUpdateFunc, version }) {
  ...
  const [formData, setFormData] = useState({
    ...data,
    call_to_action_items: data?.call_to_action_items || [],
  });
  ...
  const handleAddCallToAction = () => {
    setFormData({
      ...formData,
      call_to_action_items: [...formData.call_to_action_items, { text: "", url: "" }],
    });
  };
  ...
  const handleCallToActionChange = (index, field, value) => {
    const newCallToAction = formData.call_to_action_items.map((cta, i) =>
      i === index
        ? {
            ...cta,
            [field]: value,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res;
      if (apiUpdateFunc) {
        res = await apiUpdateFunc(toUpdate);
      } else {
