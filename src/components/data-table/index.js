...
/*  const IndeterminateCheckbox = React.forwardRef(
  ...
  const generateSortingIndicator = (column) => {
    return column.isSorted ? (column.isSortedDesc ? <ArrowDownIcon fontSize={6} className={'ml-3 text-primary'}/> :
  ...
  if (pagesLength > 1 && pagesLength <= 5) {
    for (let p = 0; p < pagesLength; p++) {
      paginationLinks.push(<Button primary className={classes('round primary', { active : p === pageIndex })} color=''
  ...
  return (
    <Fragment>
            ...
            headerGroups.map((headerGroup, hgKey) => (
              <tr key={hgKey} {...headerGroup.getHeaderGroupProps()}>
                {headerGroup.headers.map((column, hKey) => (
                  <th key={hKey} {...column.getHeaderProps([
                    {
      ...
      {
        pagination && <Row className={'justify-content-end w-100'} style={{ textAlign : 'center' }}>
          <Col sm={6}>
            <Row>
              <div className={'col-auto border-right'}>
                <p
                  className={'m-0 text-left'}>Page <strong>{pagesLength > 0 ? pageIndex + 1 : 0}</strong> of <strong>{pagesLength}</strong>
                </p>
              </div>
          ...
          <Col sm={6}>
            <div className={'row pagination-controls justify-content-end'}>
              {
                pagesLength > 1 && <Col md={2} className={''}>
                  <input
                    type='number'
                    min={1}
                    style={{ width : 70 }}
              ...
              <div className={'col-auto'}>
                {
                  (pagesLength > 5) &&
                  <Button color='' onClick={() => gotoPage(0)} disabled={!canPreviousPage}>First</Button>
                }
                <Button color='' onClick={previousPage} disabled={!canPreviousPage}>
              ...
              <div className={'col-auto'}>
                ...
                {
                  (pagesLength > 5) &&
                  <Button color='' onClick={() => gotoPage(pageCount - 1)} disabled={!canNextPage}>Last</Button>
                }
              </div>
            </div>
          </Col>
